...
from settings import TILE_SIZE, ENEMY_COLOR, SHADOW_COLOR, MAP_COLS, SHADOW_OFFSET, MAP_ROWS

class Enemy:
    def __init__(self, pos, tilemap, speed=2.0, chase_speed=3.5, detect_radius=150, lose_radius=200):
    ...
    def update(self, player_pos, player_hp=100, speed_multiplier=1.0):
        ...
        if self.state != "chase" and dist_to_player <= dynamic_detect_radius:
            self.state = "chase"
            self.path = []
            self.path_index = 0
        elif self.state == "chase" and dist_to_player > dynamic_lose_radius:
            self.state = "return"
            self.path = []
            self.path_index = 0
